...
public class Abacus extends AbstractAbacus {
	...
	private static Color MARKER_COLOR = Color.WHITE;
	private AbacusPanel panel;

	private boolean isPropertiesProvided;

	/* Holds Abacus Commands */
	private String[] commands = { "AddRod", "MinusRod", "Wait", "HighlightFrame", "HighlightRods", "HighlightBeam",
			"HighlightLowerBeads", "HighlightUpperBeads", "HighlightDots", "Reset", "Display" };
	...
	private void settingUpAbacusAttributes(String attributesFileName) throws AbacusException {
		...
		numOfRods = attributes.get("numOfRods") != null ? Integer.valueOf(attributes.get("numOfRods"))
				: Abacus.DEFAULT_NUMBER_OF_RODS;
		...
		try {
			...
			e.printStackTrace();
			throw (new AbacusException("Unable to initialize Abacus..."));
		}
	...
	private void initializeBeads(Map<String, String> attributes) throws IOException {
		...
		Integer beadWidth = attributes.get("beadWidth") != null ? Integer.valueOf(attributes.get("beadWidth"))
				: Abacus.DEFAULT_BEAD_WIDTH;
		Integer beadHeight = attributes.get("beadHeight") != null ? Integer.valueOf(attributes.get("beadHeight"))
				: Abacus.DEFAULT_BEAD_HEIGHT;
		int minusFrameWidth = frame.getFrameVerticalWidth();
		if (!canWeDisplayFrame()) {
			minusFrameWidth = 0;
		}
		...
		for (int i = 0; i < numOfRods; i++) {
			for (int j = 0; j < 5; j++) {
				...
				if (j != 4) {
					...
					beads[i][j].setDisplaylabels(true);
					beads[i][j].setPosX(frame.getFrameVerticalWidth() + ((i + 1) * rodSpace) - (beadWidth / 2)
							+ (getRods()[i].getRodWidth() / 2));
					beads[i][j].setEPosY(
							panel.getHeight() - (frame.getFrameHorizontalWidth() + 10) - ((j + 1) * beadHeight));
	...
	/**
	 * @param doWeNeedToDisplayFingers
	 *            the doWeNeedToDisplayFingers to set
	 */
